...
//
//        DepositRequestJpaEntity request = depositRequestService.createDepositRequest(
...
//
//        DepositRequestJpaEntity saveDepositRequest = depositRequestRepository.save(request);
//
...
//
//        DepositRequestJpaEntity depositRequest = findDepositRequestByRequestUuid(requestUuid);
//
...
//
//        try {
//            DepositRequestJpaEntity approvedRequest = depositRequestService.approveDepositRequest(depositRequest, adminUuid, requestDto.getMemo());
//
...
//
//            DepositRequestJpaEntity savedRequest = depositRequestRepository.save(approvedRequest);
//
...
//        DepositRequestJpaEntity depositRequest = findDepositRequestByRequestUuid(requestUuid);
//        DepositRequestJpaEntity deposit = depositRequestService.getDepositInfoByAdmin(depositRequest, admin);
//
...
//        Wallet wallet = findWalletByCustomerUuid(customer.getCustomerUuid());
//
//        Page<DepositRequestJpaEntity> depositRequests = depositRequestRepository.findByDepositPaymentUuidByStatus(customer.getCustomerUuid(), depositorName, DepositStatus.PENDING_ADMIN_APPROVAL, pageable);
//
...
//        Customer customer = customerPersistencePort.findByUserUuid(user.getUserUuid())
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", userUuid)));
//        Page<DepositRequestJpaEntity> depositRequests = depositRequestRepository.findAllByCustomerUuid(customer.getCustomerUuid(), pageable);
//
...
//        Customer customer = customerPersistencePort.findByUserUuid(userUuid)
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", userUuid)));
//        DepositRequestJpaEntity depositRequest = findDepositRequestByRequest(requestUuid, customer);
//
//        DepositRequestJpaEntity deposit = depositRequestService.getDepositInfo(depositRequest, customer);
//
...
//        List<DepositRequestJpaEntity> pendingRequests = depositRequestRepository.findByStatus(DepositStatus.PENDING_ADMIN_APPROVAL);
//        List<DepositRequestJpaEntity> expiredRequests = depositRequestService.expireOldRequests(pendingRequests);
//        List<DepositRequestJpaEntity> savedRefund = depositRequestRepository.saveAll(expiredRequests);
//
...
//
//        WithdrawRequestJpaEntity request = withdrawRequestService.createWithdrawRequest(
...
//        );
//
//        WithdrawRequestJpaEntity savedWithdrawRequest = withdrawRequestRepository.save(request);
//
...
//
//        WithdrawRequestJpaEntity withdrawRequest = findWithdrawRequestByRequestUuid(requestUuid);
//
//        try {
//            WithdrawRequestJpaEntity approvedWithdrawRequest = withdrawRequestService.approveWithdrawRequest(withdrawRequest, adminUuid, requestDto.getMemo(), withdrawRequest.getExchangeRate());
//
...
//        WithdrawRequestJpaEntity withdrawRequest = findWithdrawRequestByRequestUuid(requestUuid);
//
//        WithdrawRequestJpaEntity request = withdrawRequestService.getWithdrawInfoByAdmin(withdrawRequest, admin);
//
...
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", user.getUserUuid())));
//        Wallet wallet = findWalletByCustomerUuid(customer.getCustomerUuid());
//        Page<WithdrawRequestJpaEntity> withdraws = withdrawRequestRepository.findAllByCustomerUuid(customer.getCustomerUuid(), pageable);
//
...
//        Customer customer= customerPersistencePort.findByUserUuid(user.getUserUuid())
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", user.getUserUuid())));
//        Page<WithdrawRequestJpaEntity> withdrawRequests = withdrawRequestRepository.findByWithdrawPaymentUuidByStatus(customer.getCustomerUuid(), depositorName, WithdrawStatus.PENDING_ADMIN_APPROVAL, pageable);
...
//        Customer customer= customerPersistencePort.findByUserUuid(userUuid)
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", userUuid)));
//        WithdrawRequestJpaEntity withdrawRequest = findWithdrawRequestByRequestUuidAndUserUuid(requestUuid, customer.getCustomerUuid());
//        WithdrawRequestJpaEntity request = withdrawRequestService.getWithdrawInfo(withdrawRequest);
//
...
//
//
//        RefundRequestJpaEntity refundRequest = refundRequestService.createRefundRequest(
//                customer, wallet, orderItem, orderItem.getPriceOriginal(), adminUuid,
//                requestDto.getRefundReason(), requestDto.getRefundType());
//
//        RefundRequestJpaEntity savedRefundRequest = refundRequestRepository.save(refundRequest);
//
...
//
//        User admin = findUserByUserUuid(adminUuid);
//        RefundRequestJpaEntity request = findRefundRequestByRefundUuid(refundUuid);
//
//        try {
//            RefundRequestJpaEntity refundRequest = refundRequestService.processRefundRequest(request, admin, memo);
//            WalletResponseDto wallet = walletApplicationService.deposit(
...
//
//        User admin = findUserByUserUuid(adminUuid);
//        RefundRequestJpaEntity refundRequest = findRefundRequestByRefundUuid(refundUuid);
//
//        RefundRequestJpaEntity refund = refundRequestService.getRefundInfoByAdmin(refundRequest, admin);
...
//        Customer customer = customerPersistencePort.findByUserUuid(userUuid)
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", userUuid)));
//        RefundRequestJpaEntity request = findRefundRequestByRefundUuidAndCustomer(refundUuid, customer.getCustomerUuid());
//
//        RefundRequestJpaEntity refund = refundRequestService.getRefundInfo(request);
//
...
//        Wallet wallet = findWalletByCustomerUuid(customer.getCustomerUuid());
//
//        Page<RefundRequestJpaEntity> refunds = refundRequestRepository.findAllByCustomerUuid(customer.getCustomerUuid(), pageable);
//
...
//
//        OrderPaymentJpaEntity orderPayment = orderPaymentService.crateOrderPayment(customer, findWallet);
//        OrderPaymentJpaEntity savedOrderPayment = orderPaymentRepository.save(orderPayment);
//
...
//        Wallet wallet = findWalletByCustomerUuid(customer.getCustomerUuid());
//
//        OrderPaymentJpaEntity orderPayment = orderPaymentService.createSecondOrderPayment(customer, requestDto.getDeliveryFee(), admin, wallet);
//        OrderPaymentJpaEntity savedOrderPayment = orderPaymentRepository.save(orderPayment);
//        orderItemPersistencePort.secondOrderItemPayment(orderItem.getCustomerUuid(), requestDto, savedOrderPayment.getPaymentUuid());
//
//        OrderPaymentJpaEntity secondOrderPayment = orderPaymentService.createSecondOrderPayment(customer, requestDto.getDeliveryFee(), admin, wallet);
//
...
//        }
//        List<OrderPaymentJpaEntity> orderPayments = orderPaymentRepository.findByPaymentUuidInAndCustomerUuid(requestDto.getOrderPaymentUuids(), customer.getCustomerUuid());
//
...
//        }
//
//        List<OrderPaymentJpaEntity> invalidPayments = orderPayments.stream()
...
//        Wallet findWallet = findWalletByCustomerUuid(customer.getCustomerUuid());
//
//        List<OrderPaymentJpaEntity> invalidPaymentsByWalletUuid = orderPayments.stream()
//                .filter(payment -> payment.getWalletUuid() != findWallet.getWalletUuid())
//                .toList();
//
//
//        BigDecimal totalAmount = orderPayments.stream().map(OrderPaymentJpaEntity::getPaymentAmount)
//                .reduce(BigDecimal.ZERO, BigDecimal::add);
//
//        try {
//
...
//
//            List<OrderPaymentJpaEntity> savedOrderPayments = orderPaymentRepository.saveAll(orderPayments);
//
//
...
//        User admin = findUserByUserUuid(adminUuid);
//        OrderPaymentJpaEntity findByorderPayment = orderPaymentRepository.findByPaymentUuid(paymentUuid)
...
//        OrderPaymentJpaEntity orderPayment = orderPaymentService.getOrderPaymentInfoByAdmin(admin, findByorderPayment);
...
//        Customer customer = customerPersistencePort.findByUserUuid(userUuid)
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", userUuid)));
//        OrderPaymentJpaEntity findByOrderPayment = findOrderPaymentByCustomerUuidAndPaymentUuid(customer.getCustomerUuid(), paymentUuid);
//        OrderPaymentJpaEntity orderPayment = orderPaymentService.getOrderPaymentInfo(findByOrderPayment);
//        return paymentMapper.orderPaymentInfo(orderPayment);
//    }
//
//    /**
...
//                .orElseThrow(() -> new CustomerNotFoundException(String.format("Customer not found for UserUuid: %s", userUuid)));
//
//        Page<OrderPaymentJpaEntity> orderPaymentsByPending = orderPaymentRepository.findAllByCustomerUuidAndOrderPaymentStatusAndPaymentTypeOrderByCreatedAtDesc(customer.getCustomerUuid(), status, paymentType, pageable);
//
...
//        Wallet wallet = findWalletByCustomerUuid(customer.getCustomerUuid());
//
//        Page<OrderPaymentJpaEntity> orderPayments = orderPaymentRepository.findAllByCustomerUuid(customer.getCustomerUuid(), pageable);
//
...
//
//    // helper method
//    private DepositRequestJpaEntity findDepositRequestByRequestUuid(UUID requestUuid) {
...
//    }
//
//    private DepositRequestJpaEntity findDepositRequestByRequest(UUID requestUuid, Customer customer) {
...
//    }
//
//    private OrderPaymentJpaEntity findOrderPaymentByCustomerUuidAndPaymentUuid(UUID customerUuid, UUID paymentUuid) {
...
//    }
//
//    private WithdrawRequestJpaEntity findWithdrawRequestByRequestUuid(UUID requestUuid) {
...
//    }
//
//    private WithdrawRequestJpaEntity findWithdrawRequestByRequestUuidAndUserUuid(UUID requestUuid, UUID customerUuid) {
//        return withdrawRequestRepository.findByRequestUuidAndCustomerUuid(requestUuid, customerUuid)
//                .orElseThrow(() -> new WithdrawRequestNotFoundException(String.format("Withdraw request not found for request uuid: %s", requestUuid)));
//    }
//
//    private RefundRequestJpaEntity findRefundRequestByRefundUuid(UUID refundUuid) {
...
//    }
//
//    private RefundRequestJpaEntity findRefundRequestByRefundUuidAndCustomer(UUID refundUuid, UUID customerUuid) {
